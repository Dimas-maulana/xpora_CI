...
$(document).ready(function() {
    $('#dataTable thead .searchHeader').each( function () {
        ...
        $(this).html( '<input size="'+$(this).text().length+'" type="text" placeholder="'+title+'" />' );
    } );
    var tabel = $('#dataTable').DataTable({
        initComplete: function () {
            // Apply the search document.getElementById("dataTable_filter").lastChild.children[0].value
            this.api().columns([1,2,3,4,5,6,7,8,9]).every( function (i) {
                var that = this;
 
                $( 'input, select', $("#dataTable thead tr:eq(1) th").eq(i-1) ).on( 'keyup change clear', function () {
